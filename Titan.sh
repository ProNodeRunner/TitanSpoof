...
###############################################################################
# (5) Запуск контейнера с учетом спуфинга + генерация ключа ноды
###############################################################################
create_node() {
    local idx="$1"
    local proxy_host="$2"
    ...
    echo -e "${ORANGE}[*] Введите ключ ноды (Identity Code) для titan_node_$idx:${NC}"
    ...
    echo -e "${GREEN}[✓] Ключ ноды получен. Генерируем RSA-ключ...${NC}"

    # 🔹 Генерируем RSA-ключ
    docker exec "$CONTAINER_ID" /usr/bin/titan-edge key generate

    echo -e "${GREEN}[✓] RSA-ключ создан! Привязываем ноду...${NC}"

    # 🔹 Привязываем ноду с хэшем
