...
create_node() {
    ...
    # 🔥 Ввод ключа для привязки ноды
    echo -e "${ORANGE}[*] Введите ключ для регистрации ноды $idx (скопируйте из Titan Edge)...${NC}"
    read -p "Ключ ноды: " NODE_KEY

    # Отправляем ключ в контейнер
    echo -e "${ORANGE}[*] Привязываем ключ к ноде...${NC}"
    docker exec "$CONTAINER_ID" /usr/bin/titan-edge register --key="$NODE_KEY"

    # Проверяем, что нода зарегистрирована
    echo -e "${ORANGE}[*] Проверяем статус ноды...${NC}"
    NODE_STATUS=$(docker exec "$CONTAINER_ID" /usr/bin/titan-edge status)

    if echo "$NODE_STATUS" | grep -q "Registered"; then
        echo -e "${GREEN}[✓] Нода успешно зарегистрирована!${NC}"
    else
        echo -e "${RED}[✗] Ошибка: Нода НЕ зарегистрирована!${NC}"
        docker logs "$CONTAINER_ID"
        exit 1
    fi

    echo -e "${GREEN}[✓] Контейнер titan_node_$idx полностью готов к работе!${NC}"
}
